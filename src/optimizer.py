...
class Optimizer:
    ...
    def run_algorithm(self, mi: int, lmbd: int):
        ...
        reveal_graph.plot_cost(alg.best_specimen_after_cycles, file_name=f'v1/Plot_mi{mi}_lmbd{lmbd}')
    ...
    def optimize(self, parallelize: bool = True):
        if parallelize:
            ...
            reveal_graph.plot_multiple_lines(results, file_name=f'v1/Plot_complete')
        else:
            plot_data = []
            for mi in self.mi_set:
                for lmbd in self.lambda_set:
            ...
            reveal_graph.plot_multiple_lines(plot_data, file_name=f'v1/Plot_complete')
